...
const { stdout } = shell.exec(
  `git log v${VERSION}...v${previousVersion} --oneline --pretty=format:"- %s\n"`,
  { silent: true },
);
...
const versionHeader = `## ${VERSION} (${new Date()
  .toISOString()
  .replace(/T.*/, "")})\n`;

const versionReleaseNote = [versionHeader, "\n", commits.join("\n")].join("");

const content = [head, CHANGELOG_TAG, "\n", versionReleaseNote, tail].join("");
